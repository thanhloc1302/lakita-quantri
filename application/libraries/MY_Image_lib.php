...
class MY_Image_lib extends CI_Image_lib
{
     ...
     * @author Joost van Veen
     */
    public function thumb ($config, $path_old='', $path_new='')
    {

        // Set defaults
        $config['source_image'] = $path_old . $config['source_image'];
        $config['new_image'] = isset($config['new_image']) ? $path_new . $config['new_image'] : $config['source_image'];
    ...
    function create_thumb($filename, $path_old, $path_new, $width, $height, $ratio, $unlink)
    {
        ...
        $this->thumb($config, $path_old, $path_new);
        if($unlink)
            @unlink($path_old.$filename);
    }
