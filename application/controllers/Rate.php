...
class Rate extends CI_Controller {
    ...
    function index() {
        ...
        $data['rows'] = $this->lib_mod->load_all('rate', '', array(), $per_page, $this->uri->segment(3), array('id' => 'desc'));
    ...
    function update($id = 0) {
        ...
        if (!empty($edit) || !empty($save_edit)) {
            ...
            if (!empty($_FILES['thumbnail']['name'])) {
                ...
                $data['search'] .= ' ' . $id;
                $this->lib_mod->update('rate', array('id' => $id), $data);
        ...
        $data['row'] = $this->lib_mod->detail('rate', array('id' => $id));
        $data['id'] = $id;
    ...
    function status($id, $status) {
        ...
        if ($status)
            ...
            $status = 1;
        $data = $this->lib_mod->detail('rate', array("id" => $id));
        if (isset($data[0])) {
            $this->lib_mod->update('rate', array('id' => $id), array('status' => $status));
    ...
    function delete($items_id = array()) {
        if ($this->admin_id != 35 && $this->admin_id != 37) {
            ...
            foreach ($items_id as $id) {
                if ($id != 35) {
                    $detail = $this->lib_mod->detail('rate', array("id" => $id));
                    $name_del .= $detail[0]['name'] . ', ';
                    $this->lib_mod->delete('rate', array("id" => $id));
                }
            } if (!empty($name_del)) {
