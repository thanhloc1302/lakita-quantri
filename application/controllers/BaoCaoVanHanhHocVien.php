...
$data['end']=$day2;      
    ...
    function in() {
        ...
        $b2 = $this->db->query($b1);
        $data['student']= $b2->result_array();
        
        $c1= 'SELECT * FROM `tbl_locnt`';
        $c2=$this->db->query($c1);
        $data['locnt']=$c2->result_array();
        
        $a1 = 'SELECT DISTINCT tbl_student_courses.student_id FROM `tbl_student_courses` ';
        $a2 = $this->db->query($a1);
        $data['student_courses']  = $a2->result_array();
        
        $d1 = 'SELECT DISTINCT tbl_student_learn.student_id FROM `tbl_student_learn`';
        $d2=$this->db->query($d1);
        $data['student_learn']=$d2->result_array();
        
        $e1 = 'SELECT DISTINCT tbl_comment.student_id FROM `tbl_comment`';
        $e2=$this->db->query($e1);
        $data['comment']=$e2->result_array();
        
        $f1 = 'SELECT DISTINCT tbl_exercise.student_id FROM `tbl_exercise`';
        $f2=$this->db->query($f1);
        $data['exercise']=$f2->result_array();
