				...
				<div class="portlet box light-grey">
					...
					<div class="portlet-body">
						<div class="table-toolbar">
							<div class="btn-group">
								<a class="btn green" href="<?php echo base_url();?>category/update"><i class="fa fa-plus"></i> Thêm mới</a>
							...
							<div class="btn-group pull-right">
								<form action="<?php echo base_url();?>category/search" method="post">
		                            ...
		                            <button type="submit" onclick="window.location = window.location.protocol + '//' + window.location.host + '/' + 'category/index'; return false;" class="btn red">Hủy</button>
		                            <?php } ?>
								</form>
							</div>
						</div>
						...
						<table class="table table-striped table-bordered table-hover" id="sample_1">
							...
							<tbody>
                				<form class="form-horizontal" role="form" action="<?php echo base_url();?>category/delete" method="POST" id="form-del">
									...
									<tr class="odd gradeX">
										...
										<td>
											<a href="<?php echo base_url().'category/update/'.$value['id']; ?>">
												<?php echo word_limiter($value['name'], 25); ?>
											</a>
										</td>
										<td class="center">
											<?php $parent = $this->lib_mod->detail('category', array('id'=>$value['parent']));
										...
										<td>
											<?php if($value['status']){ ?>
												<a href="<?php echo base_url().'category/status/'.$value['id'].'/'.$value['status']; ?>" class="txt-center btn btn-sm green filter-cancel"><i class="fa fa-check"></i></a>												
											<?php }else{ ?>
												<a href="<?php echo base_url().'category/status/'.$value['id'].'/'.$value['status']; ?>" class="txt-center btn btn-sm yellow filter-cancel"><i class="fa fa-times"></i></a>												
											<?php } ?>

										</td>
										<td class="center">
											<a href="<?php echo base_url().'category/update/'.$value['id']; ?>" class="btn default btn-xs purple">
												<i class="fa fa-edit"></i> Sửa
											</a>

											<a href="<?php echo base_url().'category/delete/'.$value['id']; ?>" onclick="return confirm('Bạn chắc chắn muốn xoá bản ghi này?');" class="btn default btn-xs red">
