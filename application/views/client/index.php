			...
			<div class="col-md-12">
				...
				<div class="portlet box light-grey">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-align-justify"></i> Danh sách đối tác (<?php echo number_format($total); ?> bản ghi)
						</div>
					</div>
					...
					<div class="portlet-body">
						<div class="table-toolbar">
							<div class="btn-group">
								<a class="btn green" href="<?php echo base_url();?>client/update"><i class="fa fa-plus"></i> Thêm mới</a>
							...
							<div class="btn-group pull-right">
								<form action="<?php echo base_url();?>client/search" method="post">
									<label for="">
		                            ...
		                            <button type="submit" onclick="window.location = window.location.protocol + '//' + window.location.host + '/' + 'client/index'; return false;" class="btn red">Hủy</button>
		                            <?php } ?>
								</form>
							</div>
						</div>
						...
						<table class="table table-striped table-bordered table-hover" id="sample_1">
							...
							<tbody>
                				<form class="form-horizontal" role="form" action="<?php echo base_url();?>client/delete" method="POST" id="form-del">
									...
									<tr class="odd gradeX">
										...
										<td>
											<a href="<?php echo base_url().'client/update/'.$value['id']; ?>">
										...
										<td class="center">
											<?php echo word_limiter($value['link'], 10); ?>
										</td>
										<td>
											<?php if($value['status']){ ?>
												<a href="<?php echo base_url().'client/status/'.$value['id'].'/'.$value['status']; ?>" class="txt-center btn btn-sm green filter-cancel"><i class="fa fa-check"></i></a>												
											<?php }else{ ?>
												<a href="<?php echo base_url().'client/status/'.$value['id'].'/'.$value['status']; ?>" class="txt-center btn btn-sm yellow filter-cancel"><i class="fa fa-times"></i></a>												
											<?php } ?>

										</td>
										<td class="center">
											<a href="<?php echo base_url().'client/update/'.$value['id']; ?>" class="btn default btn-xs purple">
												<i class="fa fa-edit"></i> Sửa
											</a>

											<a href="<?php echo base_url().'client/delete/'.$value['id']; ?>" onclick="return confirm('Bạn chắc chắn muốn xoá bản ghi này?');" class="btn default btn-xs red">
