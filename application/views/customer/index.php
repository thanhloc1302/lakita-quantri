        ...
        <div class="row">
            <div class="col-md-12">
                <!-- BEGIN PAGE TITLE & BREADCRUMB-->
                 <h3 class="page-title">
                Khách hàng <small>Danh sách</small>
                </h3>
			...
			<div class="col-md-12">
				...
				<div class="portlet box light-grey">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-users"></i> Danh sách khách hàng (<?php echo number_format($total); ?> bản ghi)
						</div>
					</div>
					...
					<div class="portlet-body">
						<div class="table-toolbar">
							<div class="btn-group">
								<a class="btn green" href="<?php echo base_url();?>customer/update"><i class="fa fa-plus"></i> Thêm mới</a>
							...
							<div class="btn-group pull-right">
								<form action="<?php echo base_url();?>customer/search" method="post">
		                            ...
		                            <button type="submit" onclick="window.location = window.location.protocol + '//' + window.location.host + '/' + 'customer/index'; return false;" class="btn red">Hủy</button>
		                            <?php } ?>
								</form>
							</div>
						</div>
						...
						<table class="table table-striped table-bordered table-hover" id="sample_1">
							...
							<tbody>
                				<form class="form-horizontal" role="form" action="<?php echo base_url();?>customer/delete" method="POST" id="form-del">
									...
									<tr class="odd gradeX">
										...
										<td class="center">
                                            <img style="width:50px!important;" src="<?php if(!empty($value['thumbnail'])) echo base_url().$value['thumbnail']; else echo base_url(). 'styles/assets/img/user-default.png';?>" class="img-responsive" alt=""/>
										</td>	
										<td>
											<a href="<?php echo base_url().'customer/view/'.$value['id']; ?>">
										...
										<td class="center">
											<?php if($value['service']==0) echo 'Facebook'; else echo 'Google';?>
										</td>
										<td>
											<?php if($value['status']){ ?>
												<a href="<?php echo base_url().'customer/status/'.$value['id'].'/'.$value['status']; ?>" class="txt-center btn btn-sm green filter-cancel"><i class="fa fa-check"></i></a>												
											<?php }else{ ?>
												<a href="<?php echo base_url().'customer/status/'.$value['id'].'/'.$value['status']; ?>" class="txt-center btn btn-sm yellow filter-cancel"><i class="fa fa-times"></i></a>												
											<?php } ?>

										</td>
										<td class="center">
											<a href="<?php echo base_url().'customer/view/'.$value['id']; ?>" class="btn default btn-xs blue">
												<i class="fa fa-eye"></i> Xem
											</a>

											<a href="<?php echo base_url().'customer/update/'.$value['id']; ?>" class="btn default btn-xs purple">
												<i class="fa fa-edit"></i> Sửa
											</a>

											<a href="<?php echo base_url().'customer/delete/'.$value['id']; ?>" onclick="return confirm('Bạn chắc chắn muốn xoá bản ghi này?');" class="btn default btn-xs red">
